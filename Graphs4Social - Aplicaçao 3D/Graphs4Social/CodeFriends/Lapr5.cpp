...
void imprime_ajuda(void)
{
  ...
  printf("F2  - LÅEgrafo para ficheiro \n");
...
void desenhaArco(Arco arco){
	...
	if(nos[arco.noi].x==nos[arco.nof].x){
		...
		if(nos[arco.noi].y==nos[arco.nof].y){
			...
			cout << "arco diagonal... n„o serÅEdesenhado";
		}
	}
}
...
void setProjection(int x, int y, GLboolean picking){
    glLoadIdentity();
	if (picking) { // se estÅEno modo picking, lÅEviewport e define zona de picking
		GLint vport[4];
		glGetIntegerv(GL_VIEWPORT, vport);
		gluPickMatrix(x, glutGet(GLUT_WINDOW_HEIGHT)  - y, 4, 4, vport); // Inverte o y do rato para corresponder ÅEjana
	}
